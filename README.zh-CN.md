...
`com.gameframex.unity.tuyoogame.yooasset.alipay` 提供面向支付宝小游戏运行时的 YooAsset `IFileSystem` 适配层，将支付宝文件接口接入 YooAsset，使打包资源能够在支付宝小游戏容器内完成下发、缓存与加载。

## 功能特性

- 专为支付宝小游戏运行时定制的 `IFileSystem` 实现。
- 集成支付宝 SDK 完成 AssetBundle 下载与本地缓存。
- 通过 YooAsset 操作完成包版本请求、清单加载与资源包下载/加载。
- 可插拔的 `IRemoteServices`，支持自定义 CDN 与解密流程。
   ...
   ```json
{
   ...
   ```
3. 在 Unity 的 **Package Manager** 中使用 **Git URL** 的方式添加库，地址为：`https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.alipay.git`
`
4. 直接下载仓库放置到 Unity 项目的 `Packages` 目录下，会自动加载识别。

### 使用示例

使用内置的 `AliPayFileSystemCreater` 获取支付宝小游戏运行时的 `WebPlayModeParameters`。
...
查看 [Releases](https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.alipay/releases) 了解更新日志。
